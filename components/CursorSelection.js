  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row mb-4">
        ...
        <button className={`${styles.green} w-16 h-16 m-1 border-2 border-white rounded-md outline-none transition-colors duration-300 bg-green-500 hover:border-green-500`} onClick={() => handleCursorSelection('/green-saber.cur')}></button>
        <button className={`${styles.purple} w-16 h-16 m-1 border-2 border-white rounded-md outline-none transition-colors duration-300 bg-purple-500 hover:border-purple-500`} onClick={() => handleCursorSelection('/purple-saber.cur')}></button>
      </div>
      <div style={{ visibility: isCursorClicked ? 'visible' : 'hidden' }}>
        <button
          className={`text-sm mt-8 border-2 border-white text-white px-2 py-1 rounded-md outline-none transition-all delay-100 duration-1000 ${isCursorClicked ? 'opacity-100' : 'opacity-0'}`}
